...
int num_of_three_rows(char board[3][3], char player);

int num_of_two_rows(char board[3][3], char player);

int num_of_one_rows(char board[3][3], char player);
	...
	*/
	for (i = 0; i < 3; i++){
		for (j = 0; j < 3; j++) {			
			...
			value = 123*c3 - 63*n2 + 31*c2 - 15*n1 + 7*c1; 
			/* value = c3 - n2 + c2 - n1 + c1; */
			board[i][j] = '_';			
			if (value > best_value) {
				best_value = value;
				*row_choice = i;
